    ...
    <div className="container mx-auto py-6 px-4 md:px-6 rtl" dir="rtl">
      ...
      <Card className="mb-8">
        <CardHeader className="bg-muted/50">
          <CardTitle className="text-2xl md:text-3xl">{reportData.program.name}</CardTitle>
          <CardDescription className="text-lg">{reportData.program.institution}</CardDescription>
           <Dialog>
           ...
           <DialogContent className="sm:max-w-md" dir="rtl">
             <DialogHeader>
               <DialogTitle className="text-right">تقرير برنامج {reportData.program.courseName}</DialogTitle>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{formatDate(reportData.program.start_date)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="h-5 w-5 text-muted-foreground" />
              <div>
                ...
                <p className="font-medium">{formatDate(reportData.program.end_date)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{reportData.summary.totalTrainees}</p>
              </div>
            </div>
          </div>
            ...
            <div className="mt-4">
              <div className="flex justify-between mb-1">
                ...
                <span className="text-sm font-medium">
                  {Math.round(
                    calculatePaymentPercentage(reportData.summary.totalPaid, reportData.summary.totalPrice),
                  )}
                  %
                </span>
              </div>
              <Progress
                value={calculatePaymentPercentage(reportData.summary.totalPaid, reportData.summary.totalPrice)}
                className="h-2"
              />
            </div>
          </div>
      ...
      <Tabs defaultValue={activeEmployee} onValueChange={setActiveEmployee} className="mt-6">
        <TabsList className="mb-4 w-full flex overflow-x-auto">
          {reportData.employees.map((emp) => (
            <TabsTrigger key={emp.employee.id} value={emp.employee.id} className="flex-1 min-w-[150px]">
              {emp.employee.name}
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4" dir="rtl">
                  ...
                  <div className="flex gap-4">
                    <div className="text-center">
                      ...
                      <p className="text-xl font-bold">{emp.summary.totalTrainees}</p>
                    </div>
                    <div className="text-center">
                    ...
                    <TableBody>
                      {emp.trainees.map((trainee, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{trainee.totalPrice.toLocaleString()} دج</TableCell>
                          <TableCell className="text-red-600">{trainee.unpaidAmount.toLocaleString()} دج</TableCell>
                          <TableCell className="text-green-600">{trainee.paidAmount.toLocaleString()} دج</TableCell>
                ...
                <div className="mt-6 p-4 bg-muted/30 rounded-lg" dir="rtl">
                  ...
                  <div className="mt-4">
                    <div className="flex justify-between mb-1">
                      <span className="text-sm">نسبة التحصيل</span>
                      <span className="text-sm font-medium">
                        {Math.round(calculatePaymentPercentage(emp.summary.totalPaid, emp.summary.totalPrice))}%
                      </span>
                    </div>
                    <Progress
                      value={calculatePaymentPercentage(emp.summary.totalPaid, emp.summary.totalPrice)}
                      className="h-2"
                    />
                  </div>
                </div>
