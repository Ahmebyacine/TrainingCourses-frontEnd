...
import AttestationDeFormationPDF from "@/utils/PDF/member/AttestationDeFormation";
import { toast } from "sonner";

export default function AttestationDeFormation() {
  const {
    control,
    register,
    handleSubmit,
  ...
  async function handleGeneratePDF(values) {
    ...
    try {
      const blob = await pdf(
        <AttestationDeFormationPDF values={values} />
      ).toBlob();
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      ...
      <h1 className="text-xl font-semibold mb-6">
        Attestation de formation (sans durée)
      </h1>
