...
export const YearlyOverview = ({ selectedYear, yearlyData, loading }) => {
  ...
  const yearlyChartData = yearlyData?.statistics.map((stat) => ({
    ...
    totalAmount: stat.performance.yearlyTotal.totalAmount,
    totalPaid: stat.performance.yearlyTotal.totalPaid,
    totalUnpaid: stat.performance.yearlyTotal.totalUnpaid,
  }));

  const paymentStatusData = [
    { name: "المدفوع", value: yearlyData?.statistics.reduce((sum, stat) => sum + stat.performance.yearlyTotal.totalPaid, 0) },
    { name: "غير المدفوع", value: yearlyData?.statistics.reduce((sum, stat) => sum + stat.performance.yearlyTotal.totalUnpaid, 0) },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-right">إحصائيات الموظفين لسنة {selectedYear}</CardTitle>
