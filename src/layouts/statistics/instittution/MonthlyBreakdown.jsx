...
export const MonthlyBreakdown = ({ selectedYear, monthlyData, loading }) => {
  ...
  const processMonthlyTrend = (statistics) => {
    const aggregation = statistics.reduce((acc, institution) => {
      institution.monthlyData.forEach(monthData => {
  ...
  const getCurrentMonthStats = (statistics, targetMonth) => {
    return statistics.map(institution => {
      const monthlyEntry = institution.monthlyData.find(m => m.month === targetMonth);
      return {
