...
export default function TraineesTable({ fields, handleEditTrainee, handleDeleteTrainee }) {
    return (
      <div className="mb-8">
        <h2 className="text-lg font-semibold mb-4 pb-2 border-b">قائمة المتدربين</h2>
        {fields.length > 0 ? (
              ...
              <TableBody>
                {fields.map((field, index) => (
                  <TableRow key={field.id}>
                    <TableCell className="font-medium">{field.fullName}</TableCell>
                    <TableCell>
                      {field.birthDate ? format(field.birthDate, "PPP") : "-"}
                    </TableCell>
